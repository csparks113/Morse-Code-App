# Console Replay Summary G현 2025-10-10

- **Source**: Developer console Play Pattern (`EXPO_FORCE_NITRO_OUTPUTS=1`)
- **Unit**: 40 ms (dots) / 120 ms (dashes)
- **Highlights**:
  - `outputs.flashPulse.commit` latencies track `timelineOffsetMs` within ~5G혀15 ms across the sweep.
  - Highest observed native offset: ~90 ms (symbol index 8); corresponding commit latency 98 ms (spike trace not emittedG현below 100 ms threshold).
  - Torch channel now reports `timelineOffsetMs`; `touchToTorch` latencies range 53G혀58 ms once offsets applied.
- **Notes**:
