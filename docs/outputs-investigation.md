...
- Next replay sweep should validate whether the new 8 ms flash lead pulls audio->flash means down without introducing spike regressions; keep logging analyzer output to confirm.
- Next focus: explore native-side scheduling tweaks (e.g., shorter batch window or priority queue) now that the audio-clock alignment experiment has been rolled back.
